...
#                self.moveVect = Vector(0,0)
    ...
    def get_hit(self, colision, hit_token):
        ...
        if hit_token.type == FrameInfo.HIT_TYPE_HYPER:
            ...
            if self.moveVect != self.vector_facing((-0.4,0)): # first hyper combo hit
                self.frameCount = 15
            self.moveVect = self.vector_facing((-0.4,0))
            self.combo_count.combo(self.getState())
            self.health.loseHp(hit_token.damage, self.combo_count.getDmgReduce())
...
#        surface.convert_alpha()
    ...
    def mainloop(self):
        ...
        while True:
            ...
            stick_inputs_p1, btn_inputs_P1, stick_inputs_p2, btn_inputs_P2, special = self.input_reader.getInputs(response)
            
            print(response)
            
            self.input_reader.getInputs(response)
